...
用 `dist/visual-clipper-0.3.0-webstore.zip`（manifest 已去除 `key` 字段——商店拒收含 key 的包）。
不要用 `dist/visual-clipper-0.3.0.zip`，那个是 GitHub release 用的（保留 key 以固定扩展 ID）。
...
2. 「新建项目」→ 上传 `visual-clipper-0.3.0-webstore.zip`
